...
export async function generateMetadata({ params }: PrimeInsightDetailProps): Promise<Metadata> {
  const { slug } = await params;
  const post = await getPublishedPost(slug);

  if (!post) {
    return {
      title: 'Artikel Tidak Ditemukan | Prime Insight DRW Prime'
    };
  }

  const title = `${post.seoTitle || post.title} | Prime Insight DRW Prime`;
  const description = post.seoDescription || post.excerpt || 'Artikel Prime Insight dari DRW Prime';
  const canonical = `https://drwprime.com/prime-insight/${post.slug}`;
...
export default async function PrimeInsightDetailPage({ params }: PrimeInsightDetailProps) {
  const { slug } = await params;
  const post = await getPublishedPost(slug);
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: `https://drwprime.com/prime-insight/${post.slug}`,
    ...
    <MobileLayout>
      ...
      <main className="pt-8 md:pt-28 pb-24 md:pb-20 min-h-screen bg-dark px-4 md:px-5">
        <article className="max-w-3xl mx-auto">
          <Link href="/prime-insight" className="text-primary hover:text-primary-light transition-colors text-sm">
            ← Kembali ke Prime Insight
          </Link>
          ...
          <header className="mt-6 mb-8 border-b border-white/10 pb-6">
            ...
            {post.excerpt && <p className="text-white/70 text-sm sm:text-base">{post.excerpt}</p>}
            {post.tags.length > 0 && (
